...
def save_training_data_to_excel(data_dict, episode, goal_reached=False, steps_to_goal=np.nan, saved_model=False):
    ...
    try:
        ...
        new_row = {
            ...
            'Cumulative_Reward': data_dict['returns']  # Start new cumulative for this run
        }
        ...
        if os.path.exists(excel_path):
            try:
                ...
                if not run_data.empty:
                    cumulative_reward = run_data['Episode_Reward'].sum()
                    # Update cumulative reward for this run
                    df.loc[df['Run_ID'] == run_id, 'Cumulative_Reward'] = cumulative_reward
            except Exception as e:
                print(f"Warning: Could not read existing Excel file, creating new: {e}")
                # Create new DataFrame if reading failed
                df = pd.DataFrame([new_row])
        ...
        while episode < max_episodes and not simulation_complete:
            ...
            while not done and steps < 1000:
                ...
                training_data = {
                    ...
                    'loss': training_data.get('loss', 0.0),  # Include current loss
                    'steps': steps,
                    'epsilon': agent.epsilon
                }
                episode_rewards.append(total_reward)
                episode_steps.append(steps)
                print(f"Episode {episode}: Reward={total_reward:.2f}, Steps={steps}, Loss={training_data.get('loss', 0.0):.4f}, Epsilon={agent.epsilon:.4f}")
                
                # Save Excel data
                save_training_data_to_excel(training_data, episode, goal_reached=False)
                ...
                if done and total_reward > 0:  # Assuming positive reward for reaching goal
                    ...
                    break  # Exit episode early since goal is reached
            
            # Save model periodically
            if episode % 1000 == 0:
                torch.save(agent.qnetwork_local.state_dict(), f"dqn_model_{episode}.pth")
